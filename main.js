...
let stats = document.querySelectorAll(".stats strong");
let statsSection = document.querySelector(".stats");
let started = false;

window.addEventListener("load", () => {
  if (!started && window.scrollY + window.innerHeight >= statsSection.offsetTop) {
...
function startCount(el) {
  ...
  let increment = Math.ceil(goal / (duration / 20)); // smooth step
  ...
  let counter = setInterval(() => {
    ...
    el.textContent = formatNumber(count, goal);
  }, 20); // updates every 20ms
}

function formatNumber(num, goal) {
  if (goal >= 1000) {
...
const words = ["Ignite.", "Innovate.", "Impact."];
let wordIndex = 0;
let charIndex = 0;
let isDeleting = false;
let typingSpeed = 150; // speed per letter
let pauseBetween = 1200; // pause before deleting
...
function typeEffect() {
  ...
  // Adjust typing speed
  let speed = isDeleting ? 80 : typingSpeed;
